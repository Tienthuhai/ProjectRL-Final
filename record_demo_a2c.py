"""
record_demo_a2c.py — Quay video demo agent A2C đã train trên cả 6 map.

Chạy sau khi train_a2c.py hoàn tất và có results_a2c/models/a2c_best.zip.

Output:
  results_a2c/videos/demo_<map_name>.mp4  — video riêng cho từng map
  results_a2c/videos/demo_all_maps.mp4    — video ghép cả 6 map (nếu có opencv-python)

Yêu cầu thêm: opencv-python (pip install opencv-python)
               imageio[ffmpeg] hoặc ffmpeg trên PATH

Cách dùng:
  python record_demo_a2c.py
  python record_demo_a2c.py --model results_a2c/models/a2c_final.zip   # dùng model khác
"""
...
# ── Cấu hình ──────────────────────────────────────────────────
DEFAULT_MODEL = "results_a2c/models/a2c_best.zip"

VIDEO_DIR     = "results_a2c/videos"
FPS           = 30
...
def main():
    parser = argparse.ArgumentParser(description="Record UAV A2C Demo Videos")
