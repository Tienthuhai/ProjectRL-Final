...
# ── Cấu hình ──────────────────────────────────────────────────
LOG_CSV   = "results_a2c/train_log.csv"
CHART_DIR = "results_a2c/charts"
...
def load_data() -> pd.DataFrame:
    ...
    csv_path = LOG_CSV
    if not os.path.exists(csv_path) and os.path.exists("results_a2c/logs/train_log.csv"):
        csv_path = "results_a2c/logs/train_log.csv"
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"Không tìm thấy '{LOG_CSV}' hoặc 'results_a2c/logs/train_log.csv'. Hãy chạy train_a2c.py trước."
        )
...
def main():
    print("=" * 60)
    print("  UAV A2C — Vẽ Biểu Đồ Kết Quả Huấn Luyện")
    print("=" * 60)
