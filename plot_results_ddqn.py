...
# ── Cấu hình ──────────────────────────────────────────────────
LOG_CSV   = "results_ddqn/train_log.csv"
CHART_DIR = "results_ddqn/charts"
...
def load_data() -> pd.DataFrame:
    ...
    csv_path = LOG_CSV
    if not os.path.exists(csv_path) and os.path.exists("results_ddqn/logs/train_log.csv"):
        csv_path = "results_ddqn/logs/train_log.csv"
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"Không tìm thấy '{LOG_CSV}' hoặc 'results_ddqn/logs/train_log.csv'. Hãy chạy train_ddqn.py trước."
        )
...
def main():
    print("=" * 60)
    print("  UAV DQN — Vẽ Biểu Đồ Kết Quả Huấn Luyện")
    print("=" * 60)
