"""
record_demo_ddqn.py — Quay video demo agent DQN đã train trên cả 6 map.

Chạy sau khi train_ddqn.py hoàn tất và có results_ddqn/models/ddqn_best.zip.

Output:
  results_ddqn/videos/demo_<map_name>.mp4  — video riêng cho từng map
  results_ddqn/videos/demo_all_maps.mp4    — video ghép cả 6 map (nếu có opencv-python)

Yêu cầu thêm: opencv-python (pip install opencv-python)
               imageio[ffmpeg] hoặc ffmpeg trên PATH

Cách dùng:
  python record_demo_ddqn.py
  python record_demo_ddqn.py --model results_ddqn/models/ddqn_final.zip   # dùng model khác
"""
...
# ── Cấu hình ──────────────────────────────────────────────────
DEFAULT_MODEL = "results_ddqn/models/ddqn_best.zip"

VIDEO_DIR     = "results_ddqn/videos"
FPS           = 30
...
def main():
    parser = argparse.ArgumentParser(description="Record UAV DQN Demo Videos")
